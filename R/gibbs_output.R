...
gibbsFun <- function(data, n.iter, n.burnin, thin, estimates, interval, item.dropped, pairwise){
  p <- ncol(data)
  res <- list()
  if ("alpha" %in% estimates || "lambda2" %in% estimates || "lambda4" %in% estimates || "lambda6" %in% estimates ||
      "glb" %in% estimates){
    C <- covSamp(data, n.iter, n.burnin, thin, pairwise)
    if (item.dropped) {
  ...
  if ("omega" %in% estimates){
    om_samp <- omegaSampler(data, n.iter, n.burnin, thin, pairwise)
    ...
    if (item.dropped){
      ...
      for (i in 1:p){
        tmp <- data[-i, -i]
        om_samp_ifitem[, i] <- omegaSampler(tmp, n.iter, n.burnin, thin, pairwise)$omega
      }
